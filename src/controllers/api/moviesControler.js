        ...
        let newMovie = await db.Movie.create({
          ...
          if(newMovie) {
            response = {
                ...
                meta: {
                    status: 200,
                    url : getURL(req) + '/' + newMovie.id  // getURL(req) funcion creada mas arriba
                },
                data: newMovie
              }
            return res.status(200).json(response)
    ...
    try {
      ...
      let moviesIds = movies.map(movie => movie.id)
        ...
        if(!moviesIds.includes(+req.params.id)){
          let error = new Error('ID de movie inexistente')
          error.status = 404
          throw error
        }
      ...
      if(statusUpdate[0] === 1){
          response = {
            ...
            msg: 'Los cambios fueron realizados con exito'
          }
